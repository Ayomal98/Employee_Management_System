...
export const UpdateEmployee = () => {
  ...
  const updateForm =async(e)=>{
    e.preventDefault()
    try {
        const result=await fetch(`http://localhost:8080/api/v1/employee/${employeeId}`,
                    {method:'PUT',
    ...
    <div className='flex shadow border-b max-w-2xl mx-auto'>
        <form className='px-8 py-8'>
            ...
            <div className='items-center justify-center space-x-4 h-14 w-full my-4'>
                <button onClick={updateForm} className='rounded text-white font-semibold bg-green-400 hover:bg-green-700 py-2 px-6 mt-2'>Update</button>
                <button onClick={goBackHandler} className='rounded text-white font-semibold bg-red-400 hover:bg-red-700 py-2 px-6 mt-2'>Go Back</button>
            </div>
        </form>
    </div>
  )
